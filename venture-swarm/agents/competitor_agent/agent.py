...
from agents.competitor_agent.prompts import SYSTEM_PROMPT
from agents.competitor_agent.seeds import SEED_ITEMS
...
install_shutdown_handlers("competitor-agent", _stop_runtime)
...
async def lifespan(_: FastAPI):
    ...
    agent = build_sdk_agent(
        ...
        default_name="competitor-agent",
        default_description="Competitor analysis agent",
        default_port=9103,
    ...
    _process_message = build_startup_task_processor(
        ...
        base_notes=["Uses configured LLM provider for live competitor reasoning when available."],
    )

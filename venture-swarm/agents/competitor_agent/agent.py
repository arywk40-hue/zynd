...
async def lifespan(_: FastAPI):
    ...
    agent = ZyndAIAgent(
        AgentConfig(
            name=_agent_config.get("name", "competitor-agent"),
            description=_agent_config.get("description", "Competitor analysis agent"),
            category=_agent_config.get("category", "market-intelligence"),
            tags=_agent_config.get("tags", ["competition"]),
            summary=_agent_config.get("summary", "Competitor intelligence"),
            capabilities=_agent_config.get("capabilities", {"skills": ["competitor-analysis"]}),
            webhook_port=int(_agent_config.get("webhook_port", 9103)),
    ...
    agent.add_message_handler(lambda message, _: log.info("[Competitor] Message received from %s", message.sender_id))
